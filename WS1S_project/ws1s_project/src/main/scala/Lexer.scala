...
object Lexer extends RegexParsers {
  ...
  def id: Parser[Identifier] = positioned { "[a-zA-Z_][a-zA-Z0-9_]*".r ^^ { str => Identifier(str) } }

  def intLit: Parser[IntLiteral] = positioned { "-?[0-9]+".r ^^ { s => IntLiteral(s.toInt)} }

  def or = positioned { ("||" | "\\/") ^^^ Or()}
  def and = positioned { ("&&" | "/\\") ^^^ And()}
  def not = positioned { ("!" | "~") ^^^ Not()}
  def equals = positioned { ("==") ^^^ Equals()}
